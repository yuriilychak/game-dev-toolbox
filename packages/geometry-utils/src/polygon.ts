...
export default class Polygon {
  ...
  public bind(
    data: Float64Array,
    offset: number = 0,
    pointCount: number = data.length >> 1
  ): void {
  ...
  public pointIn(point: Point, offset: Point = null): boolean {
    ...
    for (i = 0; i < pointCount; ++i) {
      ...
      if (
        currPoint.almostEqual(innerPoint) ||
                innerPoint.onSegment(currPoint, prevPoint)
      ) {
        return null;
      }
  ...
  public normalize(): Float64Array {
    ...
    if (this.pointCount !== pointCount) {
      this.pointCount = pointCount;
      this.memSeg = this.memSeg.slice(
        this.offset,
        this.offset + (pointCount << 1)
      );
    }
  ...
  private calculateBounds(): void {
    ...
    for (i = 0; i < this.pointCount; ++i) {
      ...
      if (
        !(
          (almostEqual(point.x, point1.x) ||
                        almostEqual(point.x, point2.x)) &&
                    (almostEqual(point.y, point1.y) ||
                        almostEqual(point.y, point2.y))
        )
      ) {
        this.rectangle = false;
        break;
