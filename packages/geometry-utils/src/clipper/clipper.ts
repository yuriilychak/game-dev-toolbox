...
export default class Clipper extends ClipperBase {
  ...
  private processEdgesAtTopOfScanbeam(topY: number): void {
    ...
    while (edge1 !== null) {
      ...
      if (isMaximaEdge) {
        ...
        if (
          edge1.getIntermediate(topY) &&
                    edge1.NextInLML.isHorizontal
        ) {
        ...
        if (this.StrictlySimple) {
          ...
          if (
            ...
            outPt1 = OutRec.addOutPt(
              this.polyOuts,
              edge2,
              edge1.Curr
            );
            outPt2 = OutRec.addOutPt(
              this.polyOuts,
              edge1,
              edge1.Curr
            );
    ...
    while (edge1 !== null) {
      if (edge1.getIntermediate(topY)) {
        outPt1 = edge1.isAssigned
          ? OutRec.addOutPt(this.polyOuts, edge1, edge1.Top)
          : null;
        edge1 = this.updateEdgeIntoAEL(edge1);
  ...
  private insertLocalMinimaIntoAEL(botY: number): void {
    ...
    while (this.currentLM !== null && this.currentLM.y === botY) {
      ...
      if (leftBound === null) {
        ...
        this.activeEdges = leftBound.insertEdgeIntoAEL(
          this.activeEdges
        );
      ...
      if (rightBound !== null) {
        if (rightBound.isHorizontal) {
          this.sortedEdges = rightBound.addEdgeToSEL(
            this.sortedEdges
          );
        } else {
          this.scanbeam = Scanbeam.insert(
            rightBound.Top.y,
            this.scanbeam
          );
        }
      ...
      if (
        ...
        for (i = 0; i < joinCount; ++i) {
          ...
          if (
            ...
            this.joins.push(
              new Join(join.OutPt1, outPt, join.OffPt)
            );
          }
        }
      }
      ...
      if (
        ...
        const Op2: NullPtr<OutPt> = OutRec.addOutPt(
          this.polyOuts,
          leftBound.PrevInAEL,
          leftBound.Bot
        );
  ...
  private intersectEdges(
    ...
    const edge1Stops: boolean =
            !isProtect &&
            edge1.NextInLML === null &&
            edge1.Top.almostEqual(point);
    const edge2Stops: boolean =
            !isProtect &&
            edge2.NextInLML === null &&
            edge2.Top.almostEqual(point);
      ...
      if (edge1.PolyTyp !== edge2.PolyTyp) {
        ...
        switch (this.clipType) {
          ...
          case CLIP_TYPE.DIFFERENCE:
            if (
              (edge1.PolyTyp === POLY_TYPE.CLIP &&
                                Math.min(e1Wc2, e2Wc2) > 0) ||
                            (edge1.PolyTyp === POLY_TYPE.SUBJECT &&
                                Math.max(e1Wc2, e2Wc2) <= 0)
            ) {
              this.AddLocalMinPoly(edge1, edge2, point);
            }
            break;
    ...
    if (
      edge1Stops !== edge2Stops &&
            ((edge1Stops && edge1.isAssigned) ||
                (edge2Stops && edge2.isAssigned))
    ) {
      TEdge.swapSides(edge1, edge2);
      TEdge.swapPolyIndexes(edge1, edge2);
    }
  ...
  private AddLocalMinPoly(edge1: TEdge, edge2: TEdge, point: Point) {
    ...
    if (edge2.isHorizontal || edge1.Dx > edge2.Dx) {
      ...
      edge = edge1;
      edgePrev =
                edge.PrevInAEL === edge2 ? edge2.PrevInAEL : edge.PrevInAEL;
    } else {
      ...
      edge = edge2;
      edgePrev =
                edge.PrevInAEL === edge1 ? edge1.PrevInAEL : edge.PrevInAEL;
    }
    ...
    if (
      ...
      const outPt: NullPtr<OutPt> = OutRec.addOutPt(
        this.polyOuts,
        edgePrev,
        point
      );
  ...
  protected reset(): void {
    ...
    this.scanbeam =
            this.minimaList !== null ? this.minimaList.getScanbeam() : null;
  ...
  private processHorizontal(horzEdge: TEdge, isTopOfScanbeam: boolean) {
    ...
    if (horzEdge.NextInLML !== null) {
      if (horzEdge.isAssigned) {
        const op1: NullPtr<OutPt> = OutRec.addOutPt(
          this.polyOuts,
          horzEdge,
          horzEdge.Top
        );
        ...
        const prevEdge: NullPtr<TEdge> = horzEdge.PrevInAEL;
        const nextEdge: NullPtr<TEdge> = horzEdge.NextInAEL;
        ...
        if (
          ...
          const op2: OutPt = OutRec.addOutPt(
            this.polyOuts,
            prevEdge,
            horzEdge.Bot
          );
  ...
  private prepareHorzJoins(horzEdge: TEdge, isTopOfScanbeam: boolean) {
    ...
    if (isTopOfScanbeam) {
      ...
      const offPoint: Point = outPt.point.almostEqual(horzEdge.Top)
        ? horzEdge.Bot
        : horzEdge.Top;
  ...
  private fixupIntersectionOrder(): boolean {
    ...
    for (i = 0; i < intersectCount; ++i) {
      if (!this.intersections[i].edgesAdjacent) {
        ...
        while (
          j < intersectCount &&
                    !this.intersections[j].edgesAdjacent
        ) {
          ++j;
        }
      ...
      this.SwapPositionsInSEL(
        this.intersections[i].Edge1,
        this.intersections[i].Edge2
      );
    }
      ...
      while (edge.NextInSEL !== null) {
        ...
        if (edge.Curr.x > nextEdge.Curr.x) {
          ...
          this.intersections.push(
            new IntersectNode(edge, nextEdge, point)
          );
