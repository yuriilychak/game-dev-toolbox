...
export default class OutRec {
  ...
  public fixupOutPolygon(
    ...
    while (true) {
      if (
        outPt !== null &&
                (outPt.prev === outPt || outPt.prev === outPt.next)
      ) {
        outPt.dispose();
        this.Pts = null;
  ...
  public get pointCount(): number {
    return this.Pts !== null && this.Pts.prev !== null
      ? this.Pts.prev.pointCount
      : 0;
  }
  ...
  public get area(): number {
    ...
    do {
      result =
                result +
                (outPt.prev.point.x + outPt.point.x) *
                    (outPt.prev.point.y - outPt.point.y);
      outPt = outPt.next;
    } while (outPt != this.Pts);
  ...
  public simplify(outPt: OutPt, output: OutRec[]): void {
    ...
    do //for each Pt in Polygon until duplicate found do ...
    {
      ...
      while (op2 !== this.Pts) {
        if (
          outPt.point.almostEqual(op2.point) &&
                    op2.next != outPt &&
                    op2.prev != outPt
        ) {
