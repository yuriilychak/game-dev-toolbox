...
export default class TEdge {
  ...
  public findNextLocMin(): TEdge {
    ...
    while (true) {
      while (
        !result.Bot.almostEqual(result.Prev.Bot) ||
                result.Curr.almostEqual(result.Top)
      ) {
        result = result.Next;
      }
  ...
  public topX(y: number): number {
    ...
    return y === this.Top.y
      ? this.Top.x
      : this.Bot.x + clipperRound(this.Dx * (y - this.Bot.y));
  }
  ...
  public get maximaPair(): NullPtr<TEdge> {
    ...
    if (
      this.Next !== null &&
            this.Next.Top.almostEqual(this.Top) &&
            this.Next.NextInLML === null
    ) {
      result = this.Next;
    } else if (
      this.Prev !== null &&
            this.Prev.Top.almostEqual(this.Top) &&
            this.Prev.NextInLML === null
    ) {
      result = this.Prev;
    }
  ...
  public getContributing(
    clipType: CLIP_TYPE,
    fillType: POLY_FILL_TYPE
  ): boolean {
    const isReverse: boolean =
            clipType === CLIP_TYPE.DIFFERENCE &&
            this.PolyTyp === POLY_TYPE.CLIP;

    switch (fillType) {
      case POLY_FILL_TYPE.NON_ZERO:
        return (
          Math.abs(this.WindCnt) === 1 &&
                    isReverse !== (this.WindCnt2 === 0)
        );
  ...
  public insertsBefore(edge: TEdge): boolean {
    if (this.Curr.x === edge.Curr.x) {
      return this.Top.y > edge.Top.y
        ? this.Top.x < edge.topX(this.Top.y)
        : edge.Top.x > this.topX(edge.Top.y);
    }
  ...
  public insertEdgeIntoAEL(
    activeEdge: NullPtr<TEdge>,
    startEdge: NullPtr<TEdge> = null
  ): TEdge {
    if (activeEdge === null) {
      this.PrevInAEL = null;
      this.NextInAEL = null;
  ...
  public get horzDirection(): Float64Array {
    return new Float64Array(
      this.Bot.x < this.Top.x
        ? [DIRECTION.RIGHT, this.Bot.x, this.Top.x]
        : [DIRECTION.LEFT, this.Top.x, this.Bot.x]
    );
  }
  ...
  public setWindingCount(activeEdge: TEdge, clipType: CLIP_TYPE): void {
    ...
    while (
      edge !== null &&
            (edge.PolyTyp !== this.PolyTyp || edge.isWindDeletaEmpty)
    ) {
      edge = edge.PrevInAEL;
    }
    ...
    if (edge === null) {
      ...
      if (edge.WindCnt * edge.WindDelta < 0) {
        ...
        if (Math.abs(edge.WindCnt) > 1) {
          ...
          this.WindCnt =
                        edge.WindDelta * this.WindDelta < 0
                          ? edge.WindCnt
                          : edge.WindCnt + this.WindDelta;
        } else {
          this.WindCnt = this.isWindDeletaEmpty ? 1 : this.WindDelta;
        }
      } else {
        ...
        if (this.isWindDeletaEmpty) {
          this.WindCnt =
                        edge.WindCnt < 0 ? edge.WindCnt - 1 : edge.WindCnt + 1;
        } else {
          this.WindCnt =
                        edge.WindDelta * this.WindDelta < 0
                          ? edge.WindCnt
  ...
  public static intersectPoint(
    ...
    if (
      ...
      const point: Point =
                edge2.Bot.y > edge1.Bot.y ? edge2.Bot : edge1.Bot;
    ...
    if (edge1.Delta.x === 0) {
      intersectPoint.set(
        ...
        edge2.isHorizontal
          ? edge2.Bot.y
          : clipperRound(
            (edge1.Bot.x - edge2.Bot.x) / edge2.Dx + edge2.Bot.y
          )
      );
      ...
      intersectPoint.set(
        ...
        edge1.isHorizontal
          ? edge1.Bot.y
          : clipperRound(
            (edge2.Bot.x - edge1.Bot.x) / edge1.Dx + edge1.Bot.y
          )
      );
    } else {
      ...
      intersectPoint.set(
        Math.abs(edge1.Dx) < Math.abs(edge2.Dx)
          ? clipperRound(edge1.Dx * q + b1)
          : clipperRound(edge2.Dx * q + b2),
        clipperRound(q)
      );
    }
    ...
    if (intersectPoint.y < edge1.Top.y || intersectPoint.y < edge2.Top.y) {
      ...
      intersectPoint.set(
        Math.abs(edge1.Dx) < Math.abs(edge2.Dx)
          ? edge1.topX(intersectPoint.y)
          : edge2.topX(intersectPoint.y),
        edge2.Top.y
      );
    }
