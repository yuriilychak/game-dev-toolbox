...
export default class ClipperBase {
  ...
  public addPath(polygon: Point[], polyType: POLY_TYPE): boolean {
    ...
    while (
      lastIndex > 0 &&
            (polygon[lastIndex].almostEqual(polygon[0]) ||
                polygon[lastIndex].almostEqual(polygon[lastIndex - 1]))
    ) {
      --lastIndex;
    }
    ...
    this.isUseFullRange = Point.rangeTest(
      polygon[lastIndex],
      this.isUseFullRange
    );

    edges[0].init(edges[1], edges[lastIndex], polygon[0]);
    edges[lastIndex].init(
      edges[0],
      edges[lastIndex - 1],
      polygon[lastIndex]
    );
    ...
    while (true) {
      ...
      locMin.leftBound.WindDelta =
                locMin.leftBound.Next === locMin.rightBound ? -1 : 1;
      locMin.rightBound.WindDelta = -locMin.leftBound.WindDelta;

      edge = this.ProcessBound(locMin.leftBound, isClockwise);

      const edge2: TEdge = this.ProcessBound(
        locMin.rightBound,
        !isClockwise
      );
  ...
  private ProcessBound(edge: TEdge, isClockwise: boolean) {
    ...
    if (edge.isDxHorizontal) {
      ...
      const startX: number = isClockwise
        ? edge.Prev.Bot.x
        : edge.Next.Bot.x;
