...
export default class ClipperOffset {
  ...
  private doOffset(delta: number): Point[] {
    ...
    if (pointCount == 1) {
      ...
      for (i = 0; i < 4; ++i) {
        result.push(
          Point.from(point).scaleUp(delta).add(this.srcPolygon[0])
        );
      ...
      for (i = 0; i < pointCount; ++i) {
        normals[i] = Point.from(
          this.srcPolygon[cycleIndex(i, this.srcPolygon.length, 1)]
        )
    ...
    if (sinA * delta < 0) {
      ...
      if (r >= 1.8) {
        ...
        const dx: number = Math.tan(
          Math.atan2(sinA, normal2.dot(normal1)) * 0.25
        );
