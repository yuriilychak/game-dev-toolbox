...
export default class Join {
  ...
  constructor(
    outPt1: NullPtr<OutPt> = null,
    outPt2: NullPtr<OutPt> = null,
    offPoint: NullPtr<Point> = null
  ) {
    this.OutPt1 = outPt1;
    this.OutPt2 = outPt2;
  ...
  public joinPoints(
    ...
    if (
      isHorizontal &&
            this.OffPt.almostEqual(this.OutPt1.point) &&
            this.OffPt.almostEqual(this.OutPt2.point)
    ) {
      ...
      const value: Point = Join.getOverlap(
        op1.point.x,
        op1b.point.x,
        op2.point.x,
        op2b.point.x
      ...
      if (
        op1b === op1 ||
                op2b === op2 ||
                op1b === op2b ||
                (isRecordsSame && reverse1 === reverse2)
      ) {
        return false;
      }
  ...
  public joinCommonEdges(
    records: OutRec[],
    isUseFullRange: boolean,
    isReverseSolution: boolean
  ): void {
    const outRec1: NullPtr<OutRec> = OutRec.getOutRec(
      records,
      this.OutPt1.index
    );
    let outRec2: NullPtr<OutRec> = OutRec.getOutRec(
      records,
      this.OutPt2.index
    );
    ...
    if (
      outRec1.Pts === null ||
            outRec2.Pts === null ||
            !this.joinPoints(outRec1 === outRec2, isUseFullRange)
    ) {
      return;
    }
  ...
  public static getOverlap(
    ...
    return b1 < b2
      ? Point.create(Math.max(a2, b1), Math.min(a1, b2))
      : Point.create(Math.max(a2, b2), Math.min(a1, b1));
  }
