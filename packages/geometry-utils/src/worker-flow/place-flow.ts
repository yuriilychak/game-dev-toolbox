  ...
  while (placeContent.nodeCount > 0) {
    ...
    for (i = 0; i < placeContent.nodeCount; ++i) {
      ...
      if (
        !binNfp ||
                binNfp.length < 3 ||
                placeContent.getNfpError(placed, node)
      ) {
        continue;
      }
      ...
      finalNfp = ClipperWrapper.getFinalNfps(
        pointPool,
        placeContent,
        placed,
        node,
        ...
        for (k = 0; k < nfpSize; ++k) {
          ...
          if (
            Number.isNaN(minArea) ||
                        curArea < minArea ||
                        (almostEqual(minArea, curArea) &&
                            (Number.isNaN(minX) || tmpPoint.x < minX))
          ) {
            minArea = curArea;
