...
export default class PlaceContent extends WorkerContent {
  ...
  public init(buffer: ArrayBuffer): this {
    ...
    const mapBufferSize: number = view.getFloat64(
      Float64Array.BYTES_PER_ELEMENT * 3
    );
    const nestConfig: number = view.getFloat64(
      Float64Array.BYTES_PER_ELEMENT
    );

    this.initNodes(
      buffer,
      Float64Array.BYTES_PER_ELEMENT * 4 + mapBufferSize
    );

    this._rotations = getBits(nestConfig, 9, 5);
    this._area = view.getFloat64(Float64Array.BYTES_PER_ELEMENT * 2);
    this._nfpCache = PlaceContent.deserializeBufferToMap(
      buffer,
  ...
  public getBinNfp(index: number): Float64Array | null {
    const key: number = generateNFPCacheKey(
      this.rotations,
      true,
      this._emptyNode,
      this.nodeAt(index)
    ...
    return this._nfpCache.has(key)
      ? new Float64Array(this._nfpCache.get(key))
      : null;
  }
  ...
  public getPathKey(index: number): number {
    return joinUint16(
      toRotationIndex(this.nodeAt(index).rotation, this.rotations),
      this.nodeAt(index).source
    );
  }
  ...
  private static deserializeBufferToMap(
    buffer: ArrayBuffer,
