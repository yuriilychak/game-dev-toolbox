...
function getSegmentStats({
  ...
  if (
    point.onSegment(segmentStart, segmentEnd) ||
        point.almostEqual(target)
  ) {
...
function intersect(
  ...
  for (i = 0; i < pointCountA - 1; ++i) {
    ...
    for (j = 0; j < pointCountB - 1; ++j) {
      ...
      if (
        segmentStats ||
                (segmentStats === null && lineIntersect(b1, b2, a1, a2))
      ) {
  ...
  if (!infinite) {
    ...
    if (
      almostEqual(pdot, s1dot) &&
            ...
            midValue(pdotnorm, s1dotnorm, s2dotnorm) > 0
    ) {
      pointPool.malloc(pointIndices);
...
function coincedentDistance(
  ...
  const result: number = pointDistance(
    pointPool,
    point1,
    point3,
    point4,
...
function segmentDistance(
  ...
  const normal = pointPool
    .get(sharedPointIndices, 0)
    .update(direction)
    .normal();
  const reverse = pointPool
    .get(sharedPointIndices, 1)
    .update(direction)
    .reverse();
  ...
  if (almostEqual(crossABE) && almostEqual(crossABF)) {
    ...
    const normAB = pointPool
      .get(pointIndices3, 0)
      .update(B)
      .sub(A)
      .normal()
      .normalize();
    const normEF = pointPool
      .get(pointIndices3, 1)
      .update(F)
      .sub(E)
      .normal()
  ...
  if (almostEqual(dotB, dotE)) {
    result = Number.isNaN(result)
      ? crossB - crossE
      : Math.min(crossB - crossE, result);
  } else if (almostEqual(dotB, dotF)) {
    result = Number.isNaN(result)
      ? crossB - crossF
      : Math.min(crossB - crossF, result);
  } else {
    result = coincedentDistance(
      pointPool,
      B,
...
function polygonProjectionDistance(
  ...
  for (i = 0; i < sizeB; ++i) {
    ...
    for (j = 0; j < sizeA - 1; ++j) {
      ...
      if (
        !Number.isNaN(d) &&
                (Number.isNaN(minProjection) || d < minProjection)
      ) {
        minProjection = d;
      }
    }
...
function searchStartPoint(
  ...
  for (i = 0; i < sizeA - 1; ++i) {
    if (markedIndices.indexOf(i) === -1) {
      ...
      for (j = 0; j < sizeB; ++j) {
        startPoint
          .update(polygonA.at(i))
          .sub(polygonB.at(cycleIndex(j, sizeB, 0)));
        ...
        v.update(polygonA.at(cycleIndex(i, sizeA, 1))).sub(
          polygonA.at(i)
        );
...
function noFitPolygon(
  ...
  const startPoint: Point = pointPool
    .get(pointIndices, 3)
    .update(polygonA.at(minIndexA))
    .sub(polygonB.at(maxIndexB));
  ...
  if (inside) {
    startPointRaw = searchStartPoint(
      pointPool,
      polygon,
      polygonA,
      polygonB,
  ...
  while (true) {
    ...
    while (counter < condition) {
      ...
      if (
        reference.almostEqual(start) ||
                getNfpLooped(nfp, reference, pointPool)
      ) {
...
export function pairData(
  ...
  for (i = 0; i < nfpSize; ++i) {
    ...
    if (
      i > 0 &&
            firstNfp.pointIn(tmpPolygon.first) &&
            tmpPolygon.area < 0
    ) {
      tmpPolygon.reverse();
    }
  ...
  if (pairContent.isUseHoles) {
    ...
    for (i = 0; i < childCount; ++i) {
      ...
      if (
        child.size.x > polygonB.size.x &&
                child.size.y > polygonB.size.y
      ) {
        ...
        const noFitCount: number = noFitPolygons
          ? noFitPolygons.length
          : 0;
