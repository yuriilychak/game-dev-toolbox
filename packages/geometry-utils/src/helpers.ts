...
export function midValue(
  ...
  return (
    Math.abs(2 * value - leftRange - rightRange) -
...
export function generateNFPCacheKey(
  ...
  for (i = 0; i < elementCount; ++i) {
    result = setBits(
      ...
      data[i],
      NFP_KEY_INDICES[i],
      NFP_KEY_INDICES[i + 1] - NFP_KEY_INDICES[i]
    );
  }
...
export function getPolygonNode(
  source: number,
  memSeg: Float64Array
): PolygonNode {
...
function calculateTotalSize(nodes: PolygonNode[], initialSize: number): number {
  return nodes.reduce<number>((result: number, node: PolygonNode) => {
    const nodeSize =
            ((Float64Array.BYTES_PER_ELEMENT + Uint32Array.BYTES_PER_ELEMENT) <<
                1) +
            node.memSeg.byteLength;
...
function serializeNodes(
  ...
  return nodes.reduce((result: number, node: PolygonNode) => {
    ...
    const memSegBytes = new Uint8Array(
      node.memSeg.buffer,
      node.memSeg.byteOffset,
      node.memSeg.byteLength
    );
...
export function serializePolygonNodes(
  nodes: PolygonNode[],
  offset: number = 0
): ArrayBuffer {
...
export function serializeMapToBuffer(map: NFPCache): ArrayBuffer {
  const totalSize: number = Array.from(map.values()).reduce(
    (acc, buffer) =>
      acc + (Uint32Array.BYTES_PER_ELEMENT << 1) + buffer.byteLength,
    0
  );
