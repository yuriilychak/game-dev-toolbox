...
export default class ClipperWrapper {
  ...
  private offsetNode(
    ...
    if (this.configuration.spacing !== 0) {
      ...
      const resultPath: Point[][] = clipperOffset.execute(
        path,
        offset * ClipperWrapper.CLIPPER_SCALE
      );
  ...
  private cleanNode(node: PolygonNode): void {
    ...
    const clearedPolygon: Point[] = cleanPolygon(
      biggest,
      curveTolerance * ClipperWrapper.CLIPPER_SCALE
    );

    pointCount =
            clearedPolygon && clearedPolygon.length ? clearedPolygon.length : 0;
  ...
  private static fromNfp(
    memSeg: Float64Array,
    index: number,
    offset: Point = null
  ): Point[] {
    const cleanTrashold: number =
            offset === null ? -1 : ClipperWrapper.CLEAN_TRASHOLD;
    ...
    return ClipperWrapper.fromMemSeg(
      memSeg,
      memOffset,
      pointCount,
      1,
  ...
  public static fromMemSeg(
    ...
    return cleanTrashold !== -1
      ? cleanPolygon(result, cleanTrashold)
      : result;
  }
  ...
  public static toMemSeg(
    ...
    const result: Float64Array = memSeg
      ? memSeg
      : new Float64Array(pointCount << 1);
    const tempPoint: Point = Point.zero();
    let i: number = 0;

    for (i = 0; i < pointCount; ++i) {
      tempPoint
        .fromClipper(polygon[i])
        .scaleDown(ClipperWrapper.CLIPPER_SCALE);
  ...
  public static applyNfps(
    clipper: Clipper,
    nfpBuffer: ArrayBuffer,
    offset: Point
  ): void {
  ...
  public static nfpToClipper(
    pointPool: PointPool,
    nfpMmSeg: Float64Array
  ): Point[][] {
  ...
  public static getFinalNfps(
    ...
    for (i = 0; i < placed.length; ++i) {
      ...
      ClipperWrapper.applyNfps(
        clipper,
        placeContent.nfpCache.get(key),
        tmpPoint
      );
    }
    ...
    if (
      !clipper.execute(ClipType.UNION, combinedNfp, PolyFillType.NON_ZERO)
    ) {
      return null;
    }
    ...
    let finalNfp: Point[][] = [];
    const clipperBinNfp: Point[][] = ClipperWrapper.nfpToClipper(
      pointPool,
      binNfp
    );
  ...
  public static AREA_TRASHOLD: number =
    0.1 * ClipperWrapper.CLIPPER_SCALE * ClipperWrapper.CLIPPER_SCALE;

  public static CLEAN_TRASHOLD: number =
    0.0001 * ClipperWrapper.CLIPPER_SCALE;
